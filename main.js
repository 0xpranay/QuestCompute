...
async function main() {
  ...
  for (let i = 0; i < taskList.length; i++) {
    ...
    const monthBackTimestamp = moment().subtract(10, "month").unix();
    do {
      switch (id) {
        case 0:
          params = {
      ...
      if (users.data.backend.length >= 1000) {
        console.log(
          chalk.black.bgCyan(">1000 addresses detected. Fetching again")
        );
      }
      pagination += 1;
    ...
    writeFile(
      ...
      (error) => {
        ...
        console.log(
          chalk.greenBright(`Task ${i} data written successfully to disk`)
        );
      }
    );
  }
